        ...
        <div class="container">
            <div class="tab-header">
                <ul class="nav nav-tabs" id="myTab" role="tablist">
                    <?php 
                     $result=$categorymodel->readWomenCategory();
                ...
                <a class="view-more d-none d-md-flex">Women's Collections</a>
            </div>

            <?php
            $cat=$categorymodel->readWomenCategory();
            if($cat->num_rows>0){
                 while($rows = $cat->fetch_assoc()){
                    ...
                    <div class="tab-pane fade show active" id="tab-one" role="tabpanel" aria-labelledby="tab-one">
                        ...
                        <h4 id="'.$cat->id.'" class="m-3">'.$categorymodel->Category.'</h4>' ?>
                          <?php
                                    $result=$productModel->readallwomencollectionbycatid($cat->id);
                                    if($result->num_rows>0){
                                        while($rows = $result->fetch_assoc()){
