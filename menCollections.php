    ...
    <section class="product-tabs section-padding position-relative wow fadeIn animated">
        ...
        <div class="container">
            <div class="tab-header">
                ...
                <a class="view-more d-none d-md-flex">Men's Collections</a>
            </div>

            <?php
            $cat=$categorymodel->readMenCategory();
            if($cat->num_rows>0){
                 while($rows = $cat->fetch_assoc()){
                    ...
                    <div class="tab-pane fade show active" id="tab-one" role="tabpanel" aria-labelledby="tab-one">
                        <div class="row product-grid-4 pt-5">
                        <h4 id="'.$cat->id.'" class="m-3">'.$categorymodel->Category.'</h4>' ?>
                          <?php
                                    $result=$productModel->readallMencollectionbycatid($cat->id);
                                    if($result->num_rows>0){
                                        while($rows = $result->fetch_assoc()){
